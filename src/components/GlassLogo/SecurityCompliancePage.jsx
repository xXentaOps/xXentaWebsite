...
export function SecurityCompliancePage({ isOpen, onClose, onContactClick }) {
  ...
  const sections = t('security.sections', SECTIONS)
  const sData = t('security.sectionData', {})
  const controlAreas = t('security.controlAreas', CONTROL_AREAS)
  const governancePhases = t('security.governancePhases', GOVERNANCE_PHASES)
  const referenceFrameworks = t('security.frameworks', REFERENCE_FRAMEWORKS)
          ...
          >
            ...
            >
              ...
              <div className="sticky -top-8 z-20 -mx-4 flex items-center justify-between border-b border-white/[0.08] bg-[#0F172B]/95 px-4 pb-5 backdrop-blur-md md:-top-12 md:-mx-6 md:px-6 md:pb-6">
                <div className="flex flex-wrap items-center gap-3 md:gap-4">
                  ...
                  <span className="text-[11px] font-normal tracking-[0.2em] text-white/50 uppercase">
                    {t('security.header', 'Security & Trust')}
                  </span>
                  <span className="hidden items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.04] px-2.5 py-0.5 text-[10px] font-light tracking-wider text-white/40 uppercase sm:inline-flex">
                    {t('security.lastUpdated', 'Updated: 5 August 2026')}
                  </span>
                </div>
                ...
                <div className="flex items-center gap-3">
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-[#2B7FFF]/40 bg-[#2B7FFF]/10 px-2.5 py-1 text-[10px] font-light tracking-wider text-[#51A2FF] uppercase">
                    ...
                    {t('security.statusBadge', 'ISO 27001 Aligned')}
                  </span>
                  ...
                  <button
                    ...
                    aria-label={t('security.closeAria', 'Close Security & Compliance Dialog')}
                  >
                    <svg
                ...
                <div className="mt-6 rounded-xl border border-[#2B7FFF]/30 bg-[#2B7FFF]/10 p-5 backdrop-blur-sm md:p-6">
                  <div className="flex flex-col gap-2.5 sm:flex-row sm:items-baseline">
                    <span className="inline-flex shrink-0 items-center gap-2 text-xs font-medium tracking-wider text-[#51A2FF] uppercase">
                      ...
                      {t('security.currentStatus', 'Our current status')}
                    </span>
              ...
              <div className="mt-10 space-y-10">
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.scope?.title || 'Scope'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-4 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    <p>{sData.scope?.intro || 'Our security programme is being developed to cover:'}</p>
                    ...
                    <p className="pt-2 text-white/50">
                      {sData.scope?.footer || 'The formal ISMS scope, boundaries, dependencies and interfaces are being documented as part of the implementation programme.'}
                    </p>
                  </div>
                </section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.controls?.title || 'Security measures and control areas'}
                    </h2>
                  </div>
                  <p className="mt-3 text-xs font-extralight text-white/60 md:text-[13px]">
                    {sData.controls?.intro || 'xXenta’s current control foundation and implementation programme address the following areas:'}
                  </p>

                  <div className="mt-6 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
                    {controlAreas.map((item, idx) => (
                      <div
                        key={idx}
                  ...
                  <p className="mt-6 text-xs font-extralight leading-relaxed text-white/50 italic">
                    {sData.controls?.footer || 'Specific safeguards may differ by service, risk and customer arrangement. Further information can be provided where appropriate and subject to confidentiality and security restrictions.'}
                  </p>
                </section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.risk?.title || 'Risk management'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-4 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    <p>{sData.risk?.intro || 'Our risk-management approach includes:'}</p>
                    ...
                    <p className="pt-2 text-white/50">
                      {sData.risk?.footer || 'The approach is intended to connect security priorities to business objectives, customer commitments and the evolving risk environment.'}
                    </p>
                  </div>
                </section>
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.privacy?.title || 'Privacy and protection of personal data'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.privacy?.paragraphs || [
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.incidents?.title || 'Incident and personal-data breach management'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.incidents?.paragraphs || [
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.suppliers?.title || 'Suppliers and sub-processors'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.suppliers?.paragraphs || [
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.governance?.title || 'Governance and continuous improvement'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-4 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    <p>
                      {sData.governance?.intro || 'Information-security responsibilities are assigned within xXenta and are being embedded in governance, product and operational decision-making. Policies, procedures, control ownership and evidence are maintained and improved through a structured cycle.'}
                    </p>
                    <p className="font-normal text-white/90">
                      {sData.governance?.subtitle || 'The next implementation phases include:'}
                    </p>
                    <div className="grid grid-cols-1 gap-2.5 sm:grid-cols-2">
                      {governancePhases.map((phase, idx) => (
                        <div
                          key={idx}
                          className="flex items-start gap-2.5 rounded-lg border border-white/[0.04] bg-white/[0.02] p-3 text-white/80"
                        >
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.roadmap?.title || 'ISO/IEC 27001 roadmap and certification status'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.roadmap?.paragraphs || [
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.regulatory?.title || 'Regulatory readiness and customer supply chains'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.regulatory?.paragraphs || [
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <span className="h-px w-6 bg-[#51A2FF]/30" />
                    <h2 className="text-base font-normal tracking-wide text-white/95 md:text-lg">
                      {sData.contact?.title || 'Transparency, assurance and contact'}
                    </h2>
                  </div>
                  <div className="mt-4 space-y-3.5 text-xs font-extralight leading-relaxed text-white/70 md:text-[13px]">
                    {(sData.contact?.paragraphs || [
                    ...
                    <div className="mt-6 flex flex-wrap items-center gap-4 pt-2">
                      ...
                      >
                        <span>{sData.contact?.buttonText || 'Contact Security Team'}</span>
                        <svg
              ...
              <div className="mt-14 border-t border-white/10 pt-8">
                <div className="flex items-center gap-2">
                  <span className="text-[11px] font-normal tracking-[0.2em] text-white/50 uppercase">
                    {t('security.referenceFramework', 'Reference framework')}
                  </span>
                </div>

                <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-3">
                  {referenceFrameworks.map((ref, idx) => (
                    <div
                      key={idx}
                ...
                <div className="mt-8 flex flex-col items-center justify-between gap-4 border-t border-white/[0.06] pt-6 sm:flex-row">
                  <span className="text-[11px] font-extralight tracking-wider text-white/30">
                    {t('security.footerCopyright', '© {year} xXenta. Information Security & Compliance.').replace('{year}', new Date().getFullYear())}
                  </span>
                  <button
                    type="button"
                    onClick={onClose}
                  ...
                  >
                    {t('security.backToWebsite', 'Back to website ↑')}
                  </button>
                </div>
              </div>
            </motion.div>
